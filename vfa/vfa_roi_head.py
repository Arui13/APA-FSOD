...
import numpy as np
import torch
from mmfewshot.detection.models import build_aggregator
...
class VFARoIHead(MetaRCNNRoIHead):
    ...
    def __init__(self, vae_dim=2048,
                 wcem: Optional[ConfigDict] = None,
                 agent_prototypes_distill: Optional[ConfigDict] = None,
                 correlation_prototypes_assign: Optional[ConfigDict] = None,
                 *args, **kargs) -> None:
        super().__init__(*args, **kargs)

        self.vae = VAE(vae_dim, vae_dim, vae_dim)

        assert wcem is not None, "missing config of `wcem`"
        self.wcem = build_aggregator(copy.deepcopy(wcem))

        assert agent_prototypes_distill is not None, "missing config of `agent_prototypes_distill`"
        self.apa = build_aggregator(copy.deepcopy(agent_prototypes_distill))

        assert correlation_prototypes_assign is not None, "missing config of `correlation_prototypes_assign`"
        self.cpa = build_aggregator(copy.deepcopy(correlation_prototypes_assign))
    ...
    def _bbox_forward_train(self, query_feats: List[Tensor],
        ...
        """
        # query_feats1 = self.wecm(query_feats[0])
        # query_feats = [query_feats1, ]
        # print(self.wecm.base_conv.bias)
