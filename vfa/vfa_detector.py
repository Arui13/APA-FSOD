...
class VFA(MetaRCNN, TestMixins):
    ...
    def forward_train(self,
        ...
        #wecm
        query_map = self.roi_head.wcem(query_feats[0])
        support_map = self.roi_head.wcem(support_feats[0])

        # Agent Prototypes Aggregation
        prototypes = self.roi_head.apa(support_map)
        aggregated_features = self.roi_head.cpa(query_map, prototypes)
        aggregated_features = (aggregated_features,)

        # stop gradient at RPN
        query_feats_rpn = [x.detach() for x in aggregated_features]
        query_feats_rcnn = aggregated_features
